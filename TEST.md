...
3. **Encryption-Only Operations**: The following operations REQUIRE encryption and will NOT work without it:
   - Listing files on /files page (GET /api/files)
   - Uploading files on /admin page (POST /api/upload/url and /api/upload/direct)
   - Downloading private files on /download/private/{file_id} page (GET /api/download/private/{file_id})
   - File management on /admin page (same /api/files endpoint)
4. **No Fallback**: These endpoints will return HTTP 400/501 errors if encryption is not properly configured
5. **Test Encryption**: 
   - Enter "abc123" (matches the `AES_KEY_HASH` in your environment)
   - Verify encrypted requests in browser developer tools (Network tab - look for `encrypted_payload` in request/response bodies)
   - Try entering wrong key to test decryption error handling and key re-prompting
   - Try accessing endpoints without AES_KEY_HASH configured to see 501 errors
...
**Encrypted Upload (Phase 3 - REQUIRED):**
This endpoint ONLY accepts encrypted payloads. Manual testing via curl is complex due to AES encryption requirements. Use the web interface instead.

```bash
# This endpoint now ONLY accepts this format:
curl -X POST "http://localhost:8000/api/upload/url" \
     -H "Content-Type: application/json" \
     -H "Authorization: Bearer $TOKEN" \
     -d '{
...
# Regular (non-encrypted) requests will return HTTP 400:
# {"detail": "This endpoint only accepts encrypted uploads"}
```
...
**Encrypted Listing (Phase 3 - REQUIRED):**
This endpoint ONLY returns encrypted responses. No `encrypted=true` parameter needed.

```bash
# This endpoint now ONLY works with encryption:
curl "http://localhost:8000/api/files" \
     -H "Authorization: Bearer $TOKEN"

# Response will ALWAYS contain encrypted_payload:
# {"encrypted_payload": "base64-encoded-encrypted-file-list"}

# If AES_KEY_HASH not configured, returns HTTP 501:
# {"detail": "This endpoint requires encryption to be configured"}
```
...
**Encrypted Download (Phase 3 - REQUIRED):**
This endpoint ONLY returns encrypted responses. No `encrypted=true` parameter needed.
...
# Response will ALWAYS be JSON with encrypted_payload:
# {"encrypted_payload": "base64-encoded-encrypted-file-info-and-data"}

# If AES_KEY_HASH not configured, returns HTTP 501:
# {"detail": "This endpoint requires encryption to be configured"}

# The encrypted payload contains: {"file_id": "...", "file_data": "base64...", "size": 123}
```
...
- ✅ **Encryption-only endpoints** - NO fallback to unencrypted mode
- ✅ **Mandatory encrypted operations**:
  - File listing (GET /api/files) - ALWAYS encrypted
  - URL uploads (POST /api/upload/url) - ONLY accepts encrypted_payload
  - Direct uploads (POST /api/upload/direct) - ONLY accepts encrypted_payload
  - Private file downloads (GET /api/download/private/{file_id}) - ALWAYS encrypted
...
- ✅ HTTP 400/501 error responses for non-encrypted requests
- ✅ Large file support (up to 250MB encrypted payload size)
